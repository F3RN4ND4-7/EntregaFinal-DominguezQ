...
export const getById = async (req, res, next) => {
  try {
    ...
    if (!response) res.status(404).json({ msg: "OMG, sorry product not found 🧌" });
...
export const create = async (req, res, next) => {
  try {
    ...
    if (!newProd) res.status(404).json({ msg: "Ups, error creating the product 🧌" });
...
export const update = async (req, res, next) => {
  try {
    ...
    if (!prodUpd) res.status(404).json({ msg: "oh...an error occur updating the product 🧌" });
...
export const remove = async (req, res, next) => {
  try {
    ...
    if (!prodDel) res.status(404).json({ msg: "We couldn't delete product 🧌" });
